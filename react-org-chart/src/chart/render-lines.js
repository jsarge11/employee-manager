...
function renderLines(config = {}) {
  ...
  const link = svg.selectAll('path.link').data(links.filter(link => link.source.id), d => d.target.id);

  // Define the curved line function
  const curve = d3.svg.diagonal().projection(d => [d.x + nodeWidth / 2, d.y + nodeHeight / 2]);

  // Define the angled line function
  const angle = d3.svg.line().x(d => d.x).y(d => d.y).interpolate('linear');

  if (lineType === 'angle') {
    // Enter any new links at the parent's previous position.
    link.enter().insert('path', 'g').attr('class', 'link').attr('fill', 'none').attr('stroke', borderColor).attr('stroke-opacity', 0.5).attr('stroke-width', 1.25).attr('d', d => {
      const linePoints = [{
    ...
    link.enter().insert('path', 'g').attr('class', 'link').attr('stroke', borderColor).attr('fill', 'none').attr('x', nodeWidth / 2).attr('y', nodeHeight / 2).attr('d', d => {
      const source = {
        x: parentNode.x0,
        y: parentNode.y0
      };
