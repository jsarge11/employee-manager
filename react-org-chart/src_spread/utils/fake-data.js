...
module.exports = function fakeData() {
  ...
  const genData = ({ depth = 0, department = '' }) => {
    ...
    const { children, totalReports } = getChildren({ depth: depth + 1, department: department || person.department }, genData);
...
const depts = ['Engineering', 'Product', 'Communications', 'Marketing', 'HR', 'Design'];
